...
export default function Dashboard() {
  ...
  return <div className="min-h-full bg-gray-50 p-6">
    ...
    <section className="rounded-xl border border-gray-200 bg-white shadow-sm lg:col-span-2"><h2 className="border-b border-gray-200 px-6 py-4 font-bold text-gray-900">Komposisi Sampah</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-gray-100 text-left text-gray-500"><th className="px-6 py-3">Jenis</th><th className="px-6 py-3 text-right">Berat</th><th className="px-6 py-3 text-right">Nilai</th></tr></thead><tbody>{data.wasteComposition.length === 0 ? <tr><td colSpan={3} className="p-8 text-center text-gray-500">Belum ada transaksi selesai.</td></tr> : data.wasteComposition.map((row) => <tr key={row.name} className="border-b border-gray-100"><td className="px-6 py-3 font-semibold">{row.name}</td><td className="px-6 py-3 text-right">{kilos(row.weightKg)}</td><td className="px-6 py-3 text-right">{money(row.amount)}</td></tr>)}</tbody></table></div></section></div>
    <section className="mt-6 rounded-xl border border-gray-200 bg-white shadow-sm"><h2 className="border-b border-gray-200 px-6 py-4 font-bold text-gray-900">Transaksi Terbaru</h2><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead><tr className="border-b border-gray-100 text-left text-gray-500"><th className="px-6 py-3">Tanggal</th><th className="px-6 py-3">Nasabah/Unit</th><th className="px-6 py-3 text-right">Berat</th><th className="px-6 py-3 text-right">Nilai</th><th className="px-6 py-3 text-right">Pencairan</th></tr></thead><tbody>{data.recentTransactions.length === 0 ? <tr><td colSpan={5} className="p-8 text-center text-gray-500">Belum ada transaksi.</td></tr> : data.recentTransactions.map((transaction) => <tr key={transaction.id} className="border-b border-gray-100"><td className="px-6 py-3">{new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(transaction.createdAt))}</td><td className="px-6 py-3 font-semibold">{transaction.memberName}</td><td className="px-6 py-3 text-right">{kilos(transaction.totalWeightKg)}</td><td className="px-6 py-3 text-right font-semibold">{money(transaction.totalAmount)}</td><td className="px-6 py-3 text-right">{transaction.payoutMethod === "SAVINGS" ? "Tabungan" : transaction.payoutMethod === "DIRECT_CASH" ? "Tunai" : "Belum dipilih"}</td></tr>)}</tbody></table></div></section>
  </div>;
}
